...
export class ProfileDetailComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this.userDishes$ = this.dishes$.pipe(
			map(dishes => dishes.filter(dish => dish.user._id === this.currentUser._id))
		);
		this.userComments$ = this.dishes$.pipe(
			map(dishes => dishes.map(dish => dish.comments.filter(comment => String(comment.author) === this.currentUser._id))),
			map(allComments => {
